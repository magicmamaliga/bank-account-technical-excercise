...
import static com.mate.jpmc.balancetracker.Batch.CAP;
...
public class AuditBatcher {
    ...
    public AuditBatcher(Account account, AuditSender auditSender) {
        this.auditQueue = account.auditQueue;
    ...
    Submission buildBatches(List<Transaction> transactions) throws AuditException {
        ...
        TreeMap<BigDecimal, Deque<Batch>> byRemaining = new TreeMap<>();
        ...
        for (Transaction transaction : transactions) {
            ...
            Map.Entry<BigDecimal, Deque<Batch>> fit = byRemaining.ceilingEntry(amount);
            if (fit == null) {
                Batch b = new Batch();
                b.add(transaction);
                batches.add(b);
                byRemaining.computeIfAbsent(b.remaining(), k -> new ArrayDeque<>()).add(b);
            } else {
                Deque<Batch> q = fit.getValue();
                Batch b = q.pollFirst();

                if (q.isEmpty())
                    byRemaining.remove(fit.getKey());

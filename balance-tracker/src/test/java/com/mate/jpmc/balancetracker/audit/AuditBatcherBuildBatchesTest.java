...
class AuditBatcherBuildBatchesTest {
    ...
    private static Transaction credit(BigDecimal absAmount) {
        if (absAmount.signum() < 0) absAmount = absAmount.abs();
        return new Transaction(UUID.randomUUID().toString(), TransactionType.CREDIT, absAmount);
    }

    private static Transaction debit(BigDecimal absAmount) {
        if (absAmount.signum() < 0) absAmount = absAmount.abs();
        return new Transaction(UUID.randomUUID().toString(), TransactionType.DEBIT, absAmount.negate());
    }

    /**
     * Tiny builder for positive credits (integers).
     */
    private static List<Transaction> creditsOf(int... ints) {
        List<Transaction> out = new ArrayList<>(ints.length);
    ...
    void rejectsNullEmptyOrTooSmall() throws Exception {
        ...
        List<Transaction> four = creditsOf(1, 2, 3, 4);
    ...
    void throwsIfSingleTransactionExceedsCap() throws Exception {
        ...
        Transaction overDebit = debit(CAP.add(BigDecimal.ONE));
    ...
    void greedyFirstFitDecreasingPacksReasonably_random100() throws Exception {
        ...
        List<Transaction> window = new ArrayList<>(100);
        for (int i = 0; i < 100; i++) {
